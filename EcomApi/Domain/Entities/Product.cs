...
public class Product
{
    ...
    protected Product()
    {
        ...
        SeoInfo = new SeoInfo(string.Empty);
        ...
        int? createdByUserId = null)
    {
        ...
        ShortDescription = shortDescription ?? TruncateDescription(description ?? string.Empty, 150);
        Sku = sku;
        MainImageUrl = mainImageUrl ?? string.Empty;
        CategoryId = categoryId;
        SupplierId = supplierId;
        ...
        string? shortDescription = null)
    {
        ...
        Sku = sku;
        BarCode = barCode ?? string.Empty;
        MainImageUrl = imageUrl ?? string.Empty;
        CategoryId = categoryId;
        SupplierId = supplierId;
        UpdatedAt = DateTime.UtcNow;
        
        // Atualizar preço
        PriceInfo.UpdateSalePrice(price);
    ...
    public void RemoveStock(int quantity, string reason, string? documentNumber = null, int? userId = null)
    {
        ...
        var previousStock = StockInfo.Quantity;
    ...
    public StockMovement Sell(int quantity, string? reason = null, string? documentNumber = null, int? userId = null)
    {
        if (HasVariants)
            throw new DomainException("Este produto tem variações. Use a variação para venda.");
    ...
    public void UpdateCostPrice(decimal newCostPrice, int? updatedByUserId = null)
    {
        if (HasVariants)
            throw new DomainException("Este produto tem variações. O preço de custo é definido por variação.");
        
        if (newCostPrice <= 0)
            throw new DomainException("Preço de custo deve ser maior que zero.");
        
        if (newCostPrice > PriceInfo.SalePrice)
            throw new DomainException("Preço de custo não pode ser maior que o preço de venda.");
        
        PriceInfo.UpdateCostPrice(newCostPrice);
    ...
    #region Métodos de Relacionamento
    ...
    #region Métodos para Repository (Setteres)

    public void SetStockQuantity(int quantity)
    {
        if (HasVariants)
            throw new DomainException("Este produto tem variações. O estoque é definido por variação.");
    ...
    public void SetCostPrice(decimal costPrice)
    {
        ...
        if (costPrice > PriceInfo.SalePrice)
            throw new DomainException("Preço de custo não pode ser maior que o preço de venda.");
        
        PriceInfo.SetCostPrice(costPrice);
    }
    ...
    {
        Metrics = metrics ?? new ProductMetrics();
    }

    public void SetSeoInfo(SeoInfo seoInfo)
    {
        SeoInfo = seoInfo ?? new SeoInfo(Name);
    }
    ...
    {
        return $"{Name} (SKU: {Sku}) - R$ {GetCurrentPrice():F2}";
    }
